...
def pair_paths(output: config.OutputLayout, base_seed: int, index: int) -> PairPaths:
    ...
    return PairPaths(index, seed, stem, os.path.join(root, image_rel),
                     os.path.join(root, label_rel), image_rel, label_rel)
...
def _validate_record(record: dict[str, Any], paths: PairPaths) -> None:
    required = {
        "index": paths.index,
        "seed": paths.seed,
        "stem": paths.stem,
...
def _pair_file_stems(output: config.OutputLayout) -> tuple[set[str], set[str]]:
    root = os.path.abspath(output.root)
    image_dir = os.path.join(root, output.images_subdir)
    label_dir = os.path.join(root, output.labels_subdir)
    ...
    return stems(image_dir, ".png"), stems(label_dir, ".txt")
...
def resume_next_index(output: config.OutputLayout, base_seed: int, count: int) -> int:
    ...
    image_stems, label_stems = _pair_file_stems(output)
    ...
    for index, record in enumerate(records):
        _validate_record(record, expected[index])
    for index in range(len(records), len(present)):
        append_completed_pair(output, expected[index], recovered=True)
    return len(present)
