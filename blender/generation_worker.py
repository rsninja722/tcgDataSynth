...
def _build_layout(scene_cfg, library, cache_dir: str, rng, enabled_options, config_path: str | None):
    params = config.load_layout_params(scene_cfg.layout.kind, config_path)
    if scene_cfg.layout.kind == "table":
        return layouts.build_table(scene_cfg, library, cache_dir, rng,
                                   allow_overlap=params["allow_overlap"]), None
    if scene_cfg.layout.kind == "floating":
        return layouts.build_floating(scene_cfg, library, cache_dir, rng,
                                      allow_overlap=params["allow_overlap"],
                                      max_shapes=params["max_shapes"]), None
    if scene_cfg.layout.kind == "binder":
        instances, extent = layouts.build_binder(scene_cfg, library, cache_dir, rng)
    ...
    if scene_cfg.layout.kind == "hand":
        instances, extent = layouts.build_hand(scene_cfg, library, cache_dir, rng)
        return instances, extent
...
def run_one(index: int, config_path: str | None = None, output_root: str | None = None) -> dict:
    ...
    next_index = resume_next_index(output, settings["base_seed"], settings["count"])
    if index != next_index:
